...
--------------------------------------
	...
	local playSounds = true  -- i know this looks like a toggle, just realized this doesnt effect target only source, dont bother changing until this is fixed
...
--------------------------------------
	...
	AddEventHandler('unlockBaitCar', function(target)
		...
		if playSounds then PlaySoundFrontend(-1, "Breaker_01", "DLC_HALLOWEEN_FVJ_Sounds", 0) end
	end)
...
--------------------------------------
	...
	function manageGPS(veh)
		...
		elseif DoesBlipExist(blip) then RemoveBlip(blip) end
	end

--------------------------------------
--		 Dash Cam Functions			--	
--	 	Credit to Xander1998 		--
...
--------------------------------------
	...
	mainMenu = NativeUI.CreateMenu("BaitCarPro", "~w~by ~r~EURoFRA1D")
	...
	function AddMenuCarControls(menu)
		controlmenu = _menuPool:AddSubMenu(menu, "BaitCar Controls >>")
	...
	function AddMenuSettings(menu)
		settingsmenu = _menuPool:AddSubMenu(menu, "Settings >>")
	...
	function AddMenuGPS(menu)
		gpsmenu = _menuPool:AddSubMenu(menu, "GPS >>")
