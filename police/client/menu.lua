...
function load_menu()
	...
	for k in ipairs (buttonsZone) do
		buttonsZone [k] = nil
	end
	...
	buttonsCitizen[#buttonsCitizen+1] = {name = i18n.translate("menu_sz_create_title"), func = 'OpenMenuZone', params = ""}
	...
	-- Speedzone
	buttonsZone[#buttonsZone+1] = {name = i18n.translate("menu_create_spdzone_title"), func = 'createzone', params = -1}
	buttonsZone[#buttonsZone+1] = {name = i18n.translate("menu_dlt_spdzone_title"), func = 'deletezone', params = -1}
...
function deletezone()
	if speedzones ~= nil then
		...
		ShowNotification("No Zones to Delete.")
	end
end
...
if config.slowPlayers then
	local zoneSpd = (config.slowSpeed / 2.236936) -- Convert from MPH to m/s
	Citizen.CreateThread(function()
		while true do
			Citizen.Wait(250)		
			if speedzones ~= nil and IsPedInAnyVehicle(PlayerPedId()) then
				...
				for k,v in pairs(speedzones) do
					...
					local z = v[3]
					local radius = 75
					local spdCoords = vector3(x, y, z)
					local dist = #(plyCoords - spdCoords)
					if (dist < radius) then
						SetEntityMaxSpeed(vehicle, zoneSpd)
					else
...
function OpenMenuZone()
	...
	anyMenuOpen.menuName = "policemenu-zones"
	anyMenuOpen.isActive = true
end
